...
World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && AccountMgr::IsPlayerAccount(s->GetSecurity()) && !HasRecentlyDisconnected(s))
    {
        ...
        sLog->outDetail ("Cola de acceso: La cuenta id %u esta en la posicion (%u) de la cola.", s->GetAccountId(), ++QueueSize);
        return;
    }
...
void World::LoadConfigSettings(bool reload)
{
    if (reload)
    {
        if (!ConfigMgr::Load())
        {
            sLog->outError("Recarga de opciones del reino fallida: imposible leer opciones desde %s.", ConfigMgr::GetFilename().c_str());
            return;
        }
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString();
    sLog->outString("Cargando mensajes del sistema...");
    ...
    sLog->outString("Cargando correcciones de spell desde datos dbc...");
    ...
    sLog->outString("Cargando los nombres de scripts...");
    sObjectMgr->LoadScriptNames();

    sLog->outString("Cargando plantillas de instancias...");
    ...
    sLog->outString("Cargando paginas de texto...");
    sObjectMgr->LoadPageTexts();

    sLog->outString("Cargando plantillas de objetos del juego...");         // must be after LoadPageTexts
    ...
    sLog->outString("Cargando textos de NPC...");
    ...
    sLog->outString("Cargando desactivaciones...");
    DisableMgr::LoadDisables();                                  // must be before loading quests and items

    sLog->outString("Cargando objetos...");                         // must be after LoadRandomEnchantmentsTable and LoadPageTexts
    sObjectMgr->LoadItemTemplates();

    sLog->outString("Cargando nombres de sets de objetos...");                // must be after LoadItemPrototypes
    ...
    sLog->outString("Cargando plantillas de criaturas...");
    ...
    sLog->outString("Cargando datos de criaturas...");
    ...
    sLog->outString("Cargando datos de objetos del juego...");
    ...
    sLog->outString("Cargando misiones...");
    sObjectMgr->LoadQuests();                                    // must be loaded after DBCs, creature_template, item_template, gameobject tables

    sLog->outString("Comprobando misiones deshabilitadas");
    ...
    sLog->outString("Cargando requerimientos de acceso...");
    ...
    sLog->outString("Cargando auras de mascotas...");
    ...
    sLog->outString("Cargando cuerpos de jugadores...");
    ...
    sLog->outString("Cargando logros...");
    ...
    sLog->outString("Cargando logros completados...");
    sAchievementMgr->LoadCompletedAchievements();

    // Delete expired auctions before loading
    sLog->outString("Eliminando subastas expiriadas...");
    sAuctionMgr->DeleteExpiredAuctionsAtStartup();

    ///- Load dynamic data tables from the database
    sLog->outString("Cargando objetos de subasta...");
    sAuctionMgr->LoadAuctionItems();
    sLog->outString("Cargando subastas...");
    sAuctionMgr->LoadAuctions();

    sGuildMgr->LoadGuilds();

    sLog->outString("Cargando equipos de arenas...");
    sArenaTeamMgr->LoadArenaTeams();

    sLog->outString("Cargando grupos...");
    sGroupMgr->LoadGroups();

    sLog->outString("Cargando nombres reservados...");
    sObjectMgr->LoadReservedPlayersNames();

    sLog->outString("Cargando objetos de juego para misiones...");
    sObjectMgr->LoadGameObjectForQuests();

    sLog->outString("Cargando maestros de batalla...");
    sBattlegroundMgr->LoadBattleMastersEntry();

    sLog->outString("Loading GameTeleports...");
    sObjectMgr->LoadGameTele();

    sLog->outString("Cargando menus Gossip...");
    sObjectMgr->LoadGossipMenu();

    sLog->outString("Cargando opciones de menus Gossip...");
    sObjectMgr->LoadGossipMenuItems();

    sLog->outString("Cargando vendedores...");
    sObjectMgr->LoadVendors();                                   // must be after load CreatureTemplate and ItemTemplate

    sLog->outString("Cargando entrenadores...");
    ...
    sLog->outString("Cargando condiciones...");
    ...
    sLog->outString("Cargando tickets para GMs...");
    ...
    sLog->outString("Cargando mensajes de autodifusion...");
    LoadAutobroadcasts();
    ...
    sLog->outString("Iniciando sistema de mapas");
    sMapMgr->Initialize();

    sLog->outString("Iniciando sistema de eventos...");
    ...
    sLog->outString("Iniciando sistema de PVP exterior...");
    ...
    ///- Initialize Warden
    sLog->outString("Cargando comprobaciones del Warden..." );
    sWardenCheckMgr->LoadWardenChecks();

    sLog->outString("Eliminando baneos expiriados...");
    ...
    sLog->outString("Iniciando bot de subastas...");
    auctionbot.Initialize();

    // possibly enable db logging; avoid massive startup spam by doing it here.
    if (sLog->GetLogDBLater())
    {
        sLog->outString("Activando informes en base de datos...");
    ...
    sLog->outString();
    sLog->outString("Reino iniciado en %u minutos y %u segundos", (startupDuration / 60000), ((startupDuration % 60000) / 1000) );
    sLog->outString();
}
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        {
            sLog->outBasic("Update time diff: %u. Jugadores online: %u.", m_updateTimeSum / m_updateTimeCount, GetActiveSessionCount());
    ...
    if (m_timers[WUPDATE_PINGDB].Passed())
    {
        ...
        sLog->outDetail("Ping a MySQL para mantener la conexion activa");
